...
uses
  SysUtils, Classes, Controls, Forms, Dialogs, StdCtrls, Menus, SynEdit,
  SynCompletion, RegExpr, LCLProc,
...
type
  ...
  TfrmTriggerEditor = class(TFrame)
    ...
    btnSave: TButton;
    lblBody: TLabel;
    SynCompletionProposalStatement: TSynCompletion;
    ...
    procedure SynCompletionProposalStatementExecute(Sender: TObject);
  ...
  if (Token = 'NEW') or (Token = 'OLD') then begin
    if comboTable.Text = '' then
      ...
      while not Columns.Eof do begin
        ...
        Proposal.ItemList.Add(Columns.Col('Field'));
        Columns.Next;
      end;
    except
    end;
...
begin
  ...
  Result := Result + 'TRIGGER '+DBObject.Connection.QuoteIdent(editName.Text)+' '+
    comboTiming.Text+' '+comboEvent.Text+
