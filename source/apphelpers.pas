...
{$I const.inc}
  ...
  function GetFileModTime(const FileName: string): TDateTime;
...
// Return modification date/time from passed file name
function GetFileModTime(const FileName: string): TDateTime;
var
  L: LongInt;
begin
  L := FileAge(FileName); // returns DOS date/time as LongInt, or -1 on error
  if L <> -1 then
...
procedure THttpDownload.SetTimeOut(Value: Cardinal);
begin
  FTimeOut := Value;
  IOTimeout := Value * 1000;
end;
...
begin
  ...
  if ResponseStatusCode <> 200 then
    raise Exception.CreateFmt(_('Got HTTP status %d from %s'), [ResponseStatusCode, FURL]);
end;
